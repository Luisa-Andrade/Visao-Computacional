...
# Filtrar a tabela com base nas linhas que serão apresentadas
   # tabela_filtrada = tabela.iloc[0:144]  # Para selecionar todas [0:1297]

# Agrupar os dados da tabela filtrada com base nas duas primeiras colunas (nome do objeto e variação do objeto)
   # objetos = tabela_filtrada.groupby([tabela_filtrada.columns[0], tabela_filtrada.columns[1]])

# Agrupar os dados da tabela com base nas duas primeiras colunas (nome do objeto e variação do objeto)
objetos = tabela.groupby([tabela.columns[0], tabela.columns[1]])
